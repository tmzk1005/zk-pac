...
const direct = "DIRECT";
const mySocks5Proxy = "SOCKS5 127.0.0.1:1080";

function shouldUseProxy(url, host) {
    var suffix;
    if (Object.hasOwnProperty.call(domains, host)) {
        return true;
    }
    var pos = host.lastIndexOf('.');
    while (pos > 0) {
        suffix = host.substring(pos + 1);
        if (Object.hasOwnProperty.call(domains, suffix)) {
            return true;
        }
...
function FindProxyForURL(url, host) {
    ...
    if (shouldUseProxy(url, host)) {
        return mySocks5Proxy;
    }
    return direct;
}
